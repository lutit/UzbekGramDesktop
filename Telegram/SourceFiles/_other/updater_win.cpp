					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								wsprintf(nameStr, L"UzbekGram Desktop");
								...
								wsprintf(icongroupStr, L"UzbekGram Desktop");
...
static const WCHAR *_programName = L"UzbekGram Desktop"; // folder in APPDATA, if current path is unavailable for writing
